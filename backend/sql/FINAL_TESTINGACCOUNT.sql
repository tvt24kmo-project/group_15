CREATE DATABASE  IF NOT EXISTS `atm_automat` /*!40100 DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci */ /*!80016 DEFAULT ENCRYPTION='N' */;
USE `atm_automat`;
...
INSERT INTO `accounts` VALUES (7,1,998000.98,'fi5566667777888898',1221312.00),(9,2,99880.00,'fi1122223333444455',0.00),(10,3,100959.00,'fi123456678912',10000.00),(11,3,98595.00,'fi9988887777666655',0.00);
...
BEGIN
    DECLARE v_cardtype VARCHAR(10);

    -- Fetch the card type for the given card number
    SELECT cardtype INTO v_cardtype
    FROM cards
    WHERE cardnumber = p_cardnumber;

    IF v_cardtype = 'Multi' THEN
        SET p_account_type = 'Multi';
    ELSE
        SET p_account_type = 'Non-Multi';
    END IF;
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
...
-- Dump completed on 2025-02-23 22:35:51
